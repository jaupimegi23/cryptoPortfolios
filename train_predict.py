...
print(coins)
df_train = pd.read_csv("features_train.csv")[65:]
df_valid = pd.read_csv("features_val.csv")
...
for coin in coins:
    ...
    #TRAINING
    df_coin = df_all[coin_cols]
    x = df_coin.drop(columns="Log_RET_{}".format(coin))[65:nrows-1]
    y = df_coin["Log_RET_{}".format(coin)][66:nrows]
    ...
    train_X = pd.concat([train_X,test_X_ALL])
    train_y = pd.concat([train_y,test_y_ALL])
